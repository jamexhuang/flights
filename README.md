...
**Option 1: Chained Selection (Recommended for true prices)**
Google Flights requires you to select legs sequentially to see the final accumulated price. The `get_flights_multicity_chained` helper automates this by executing sequential requests and grabbing the actual final booking price:
...
# This will simulate sequential choices and return the total price at the end
result = get_flights_multicity_chained(legs)
print(f"Final Booked Ticket Price: {result[-1].total_price} USD")
```

> **⚙️ Technical Note on `get_flights_multicity_chained`:**
> `fast_flights` achieves this by programmatically traversing Google's hidden `GetShoppingResults` HTTP RPC endpoint. It passes `f.req` payloads populated with each flight leg, capturing the Base64 state tokens generated by Google. The script uses Regex string-extraction to pluck these tokens out from the dynamic JSON stream without requiring slow tools like Playwright or Selenium. 
> 
> *Caveats:*
> - The parser automatically selects the first "best" flight token returned per leg. If you need highly specific airline combinations, you should manually chain tokens.
> - Delay intervals (`delay=1.0`) are highly recommended to prevent Google's backend from throwing HTTP 429 logic blocks on rapid multistep token queries.
> - **Integrations / Fallbacks are not supported**: `get_flights_multicity_chained` is strictly implemented via `primp` HTTP sessions and currently bypasses Playwright fallbacks and BrightData integrations.

**Option 2: Manual Selection (One-way chaining)**
You can also manually chain `select_flight()` calls to step through each leg yourself:
...
## What's new
- `v3.4.0` – **Automated Multi-city Token Chaining** via `get_flights_multicity_chained` module.
